...
void readImage(const char* fname, value_type* imgData_h, bool quiet = false)
{
    ...
    if (!quiet) {
        std::cout << "NO PREPROCESSING APPLIED - Raw image data used." << std::endl;
    }
    // applyBasicPreprocessing(imgData_h, quiet); // DISABLED FOR COMPARISON
}
...
{   
    ...
    std::cout << "Simple MNIST CUDNN Implementation - NO PREPROCESSING" << std::endl;
    std::cout << "Basic 2-Conv + 1-FC architecture WITHOUT preprocessing" << std::endl;
